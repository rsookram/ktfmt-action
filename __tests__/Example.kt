/** An example file to format as part of integration tests. */
@Deprecated("Foo")
public class ThisIsASampleClass : Comparable<*>, Appendable {
    ...
    fun longMethod(@Named("param1") param1: Int, param2: String) {
        @Deprecated val foo = 1
    }

    fun multilineMethod(foo: String, bar: String?, x: Int?) {
        foo.toUpperCase().trim().length
        val barLen = bar?.length() ?: x ?: -1
        if (foo.length > 0 && barLen > 0) {
            println("> 0")
        }
    }
}
